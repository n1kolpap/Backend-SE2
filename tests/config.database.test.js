...
test.serial('connectDB: uses process.env.MONGO_URI when set and logs host on success', async (t) => {
  const { connectDB, state } = t.context;

  // Arrange: stub console and process.exit to keep test output clean and prevent exit.
...
test.serial("connectDB: falls back to default localhost URI when MONGO_URI is not set", async (t) => {
  const { connectDB, state } = t.context;
...
test.serial("connectDB: on connection error logs message and calls process.exit(1)", async (t) => {
  const { connectDB, state } = t.context;
